...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        xh = (x - sample_mean) / sample_std
        out = gamma * xh + beta
        cache = x, xh, gamma, beta, sample_std, sample_mean
...
def dropout_backward(dout, cache):
    ...
    if mode == 'train':
        ...
        #######################################################################
        dx = dout * np.ones_like(mask)*mask
        #pass
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in np.arange(N):
        for c in np.arange(C):
            for i in np.arange(Hout):
                for j in np.arange(Wout):
                    out[n,c,i,j] = np.max(x[n,c,S*i:S*i+PH,S*j:S*j+PW])
    #pass
    #print(out)
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in np.arange(N):
        for c in np.arange(C):
            for i in np.arange(Hout):
                for j in np.arange(Wout):
                    a = x[n,c,S*i:S*i+PH,S*j:S*j+PW]
                    #idx = np.argmax(x[n,c,S*i:S*i+PH,S*j:S*j+PW])
